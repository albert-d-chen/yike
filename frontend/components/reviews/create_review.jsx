            ...
            <div className='popup'>
                <div className='popup_inner'>
                    <form onSubmit={this.handleSubmit} className='create-review-form'> 
                        <h1 className='create-review-header'>Write a Review!</h1>
                        <label>
                            <input className='create-review-title' type="text" value={this.state.title} onChange={this.update('title')} onClick={this.clearInput('title')}/>
                        </label>

                        <label>
                            <textarea className='create-review-body' value={this.state.body} onChange={this.update('body')} onClick={this.clearInput('body')}/>
                        </label>
                        <div className='create-review-rate'>Rate your experience!</div>
                        ...
                        <div>
                            <div className='review-button-div'>
                                <button className='create-review-button' type='submit' onClick={() => {this.props.closePopup; window.location.reload(false);}}>Submit Review</button>
                            </div>
                            <Link to={`/products/${this.state.product_id}`}><div className='review-close' type='submit' onClick={() => {this.props.closePopup; window.location.reload(false);}}></div></Link>
                        </div>
                    </form>
                </div>
            </div>
